...
def compute_vesting_trajectory(
    ...
    launch_date = np.datetime64('2020-10-15')
    end_day = int((end_date - launch_date).astype('timedelta64[D]') / np.timedelta64(1, 'D'))
    
    # Get entire daily vesting trajectory
    full_vest_dict = {
        "date": np.arange(launch_date, end_date, dtype='datetime64[D]'),
    ...
    vest_dict = df_utils.filter_by_date(full_vest_dict, start_date)
    date_vector = vest_dict.pop("date")
